// app/nonfiksi/NonFiksiClientPage.tsx
"use client";
...
interface NonFiksiClientPageProps {
  initialBuku: Buku[];
}

export default function NonFiksiClientPage({
  initialBuku,
}: NonFiksiClientPageProps) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
              <div className="relative">
                ...
                <div className="relative bg-accent/10 border border-accent/20 backdrop-blur-sm p-3 sm:p-4 rounded-2xl">
            ...
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-card border border-border/60 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all placeholder-muted-foreground text-foreground"
              />
              {searchQuery && (
                <button
          ...
          <div className="text-center py-20">
            ...
            <p className="text-foreground text-sm sm:text-base md:text-lg font-semibold mt-4">
              {searchQuery
                ...
                : "Belum ada buku non-fiksi"}
            </p>
            <p className="text-muted-foreground mt-2">
              {searchQuery
                ? "Coba kata kunci pencarian yang berbeda"
                : "Buku non-fiksi akan segera ditambahkan"}
            </p>
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="mt-4 px-6 py-2 rounded-lg bg-accent text-accent-foreground hover:bg-accent/90 transition-colors"
              >
                Hapus Filter
              </button>
            )}
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-8 pb-20">
        <div className="relative overflow-hidden bg-gradient-to-br from-accent via-accent/90 to-primary rounded-2xl px-8 md:px-16 py-16 md:py-24 text-center shadow-2xl border border-accent/20">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <p className="text-white/80 text-sm sm:text-base md:text-lg max-w-2xl mx-auto">
              Belajar dan berkembang melalui koleksi buku non-fiksi berkualitas
              tinggi kami. Dari biografi inspiratif hingga panduan praktis.
            </p>
          </div>
        </div>
      </section>
